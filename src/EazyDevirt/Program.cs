...
internal static class Program
{
    ...
    private static void Run(Options options)
    {
        ...
        var outputFilePath = ctx.Options.OutputPath.FullName + '\\' +
                             Path.GetFileNameWithoutExtension(ctx.Options.Assembly.Name) +
        ...
        ctx.Module.Write(outputFilePath,
            new ManagedPEImageBuilder(
                dnFactory));
        ctx.Console.Success($"Saved file to {outputFilePath}", VerboseLevel.None);
    }
    ...
    private static Parser BuildParser()
    {
        ...
        var verbosityOption = new Option<VerboseLevel>(new[] { "--verbose", "-v" },
            "Level of verbosity [1: Verbose, 2: Very Verbose, 3: Very Very Verbose]");
        ...
        var saveAnywayOption =
            new Option<bool>(new[] { "--save-anyway" }, "Saves output of devirtualizer even if it fails");
        saveAnywayOption.SetDefaultValue(false);

        var onlySaveDevirtedOption = new Option<bool>(new[] { "--only-save-devirted" },
            "Only saves successfully devirtualized methods (This option only matters if you use the save anyway option)");
        ...
        rootCommand.SetHandler(Run,
            new OptionsBinder(inputArgument, outputArgument, verbosityOption,
