...
internal class OpCodeMapping : StageBase
{
    ...
    private protected override bool Init()
    {
        ...
        {
            Logger.Error("Failed to find ExecuteVMMethod method!");
            return false;
        }
        ...
        {
            Logger.Error("Failed to find VM declaring type cctor method!");
            return false;
        }
        ...
        foreach (var sub in subs)
        {
            ...
            {
                Logger.Warning(
                    $"Overwriting {fieldDef.Name} type {Ctx.VMTypeFields[fieldDef].Name} with new type {typeDefOrRef.Name}",
        ...
        Logger.InfoStr("Type fields found in VM declaring type", Ctx.VMTypeFields.Count, VerboseLevel.VeryVerbose);
        ...
        {
            Logger.Error("Failed to find VM Arguments field!");
            return false;
        }

        if (argumentFieldMatches.First()[1].Operand is not SerializedFieldDefinition argumentField)
        {
            Logger.Error("VM Arguments field is not correct!");
            return false;
        }
        ...
        if (localField == null)
        {
            Logger.Error("Failed to find VM Locals field!");
            return false;
        }
    ...
    public override bool Run()
    {
        ...
        Ctx.Console.InfoStr("VM opcode dictionary method", dictMethod.MetadataToken, VerboseLevel.VeryVerbose);
        ...
        Ctx.Console.InfoStr("VM opcodes with handlers", vmOpCodes.Count, VerboseLevel.VeryVerbose);
        ...
        Ctx.Console.InfoStr("Total VM opcodes found", containerCtorOpCodes.Count, VerboseLevel.VeryVerbose);
        ...
        foreach (var opCodeFieldInstrs in containerCtorOpCodes)
        {
            ...
            if (matchingVMOpCodes.Count <= 0)
            {
                Ctx.Console.InfoStr("Unhandled VM opcode", $"{opCode}, {operandType}", VerboseLevel.VeryVerbose);
                continue;
            }
        ...
        foreach (var vmOpCode in vmOpCodes)
        {
            ...
            {
                Ctx.Console.Warning($"VM opcode [{vmOpCode}] does not have a virtual opcode!", VerboseLevel.None);
                continue;
            }
            ...
            {
                Ctx.Console.Warning($"Failed to identify VM opcode [{vmOpCode}]", VerboseLevel.VeryVeryVerbose);
            }
            ...
            Ctx.Console.Info(vmOpCode, VerboseLevel.VeryVerbose);
        }
    ...
    {
        return Ctx.VMDeclaringType.Methods.FirstOrDefault(method =>
            method is { IsPrivate: true, IsStatic: true, Parameters.Count: 1 } &&
